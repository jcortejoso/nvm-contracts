...
contract('ProvenanceRegistry', (accounts) => {
    ...
    describe('Register decentralised identifiers with attributes, fetch attributes by DID', () => {
        it('Should discover the attribute after registering it', async () => {
            const { provenanceRegistry } = await setupTest()
            const did = constants.did[0]
            const agentId = accounts[2]
            const attributes = 'Asset Creation'
            const result = await provenanceRegistry.wasGeneratedBy(did, agentId, 'creation', delegates, attributes)
            ...
            assert.strictEqual(agentId, payload._agentId)
            assert.strictEqual(activityId, payload._activityId)
            assert.strictEqual(value, payload._value)

            const payloadProv = result.logs[1].args
            assert.strictEqual(did, payloadProv._did)
        })
        ...
        it('Should find the event from the block number', async () => {
            ...
            // get owner for a did
            const owner = await provenanceRegistry.getDIDOwner(did)
