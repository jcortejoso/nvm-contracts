...
contract("EscrowPaymentCondition constructor", accounts => {
  ...
  async function setupTest({
    ...
    if (!escrowPayment) {
      ...
      await conditionStoreManager.delegateCreateRole(createRole, {
        from: owner
      })
      ...
      await escrowPayment.initialize(owner, conditionStoreManager.address, {
        from: deployer
      })
    }
  ...
  describe("init failure", () => {
    it("needed contract addresses cannot be 0", async () => {
      ...
      await assert.isRejected(
        escrowPayment.initialize(owner, constants.address.zero, {
          from: deployer
        }),
        undefined
      )
  ...
  describe("fulfill non existing condition", () => {
    it("should not fulfill if conditions do not exist", async () => {
      ...
      await assert.isRejected(
        ...
        constants.condition.reward.escrowReward.error
          .lockConditionIdDoesNotMatch
      )
    })
  })
  ...
  describe("fulfill existing condition", () => {
    it("ERC20: should fulfill if conditions exist for account address", async () => {
      ...
      const conditionLockId = await lockPaymentCondition.generateId(
        agreementId,
        hashValuesLock
      )
      ...
      const escrowConditionId = await escrowPayment.generateId(
        agreementId,
        hashValues
      )
      ...
      await token.approve(lockPaymentCondition.address, totalAmount, {
        from: sender
      })
      ...
      assert.strictEqual(
        await getBalance(token, lockPaymentCondition.address),
        0
      )
      assert.strictEqual(
        await getBalance(token, escrowPayment.address),
      ...
      assert.strictEqual(
        (
          await conditionStoreManager.getConditionState(escrowConditionId)
        ).toNumber(),
    ...
    it("ETH: should fulfill if conditions exist for account address", async () => {
      ...
      await didRegistry.registerMintableDID(
        ...
        amounts[0],
        0,
        constants.activities.GENERATED,
        ""
      )
      ...
      const balanceContractAfterLock = await getETHBalance(
        escrowPayment.address
      )
      ...
      assert.isAtLeast(
        ...
        balanceContractBefore + totalAmount
      )
      ...
      const balanceContractAfterEscrow = await getETHBalance(
        escrowPayment.address
      )
      ...
      assert.isAtMost(
        balanceSenderAfterEscrow,
        balanceSenderBefore - totalAmount
      )
      assert.isAtMost(balanceContractAfterEscrow, balanceContractBefore)
      assert.isAtLeast(
        balanceReceiverAfterEscrow,
        balanceReceiverBefore + totalAmount
      )
      await assert.isRejected(
        escrowPayment.fulfill(
          agreementId,
    ...
    it("release condition not fulfilled", async () => {
      ...
      const lockConditionId = await lockPaymentCondition.generateId(
        agreementId,
        hashValuesLock
      )
      ...
      const hashValuesLock2 = await lockPaymentCondition.hashValues(
        did,
        escrowPayment.address,
        token.address,
        amounts2,
      ...
      const releaseConditionId = await lockPaymentCondition.generateId(
        agreementId,
        hashValuesLock2
      )
    ...
    it("ETH: release condition aborted", async () => {
      ...
      assert.strictEqual(await getETHBalance(sender), balanceBefore)
    })
    ...
    it("should not fulfill in case of null addresses", async () => {
      ...
      const conditionId = await escrowPayment.generateId(
        agreementId,
        hashValues
      )
  ...
  describe("only fulfill conditions once", () => {
    it("do not allow rewards to be fulfilled twice", async () => {
      ...
      for (let i = 0; i < escrowPaymentBalance / amounts; ++i) {
        ...
        const attackerConditionId = await escrowPayment.generateId(
          attackerAgreementId,
          attackerHashValues
        )
    ...
    it("ERC20: should bit fulfill if was already fulfilled", async () => {
      ...
      const balanceContractBefore = await getBalance(
        token,
        escrowPayment.address
      )
      ...
      assert.strictEqual(
        await getBalance(token, receivers[0]),
        balanceReceiverBefore + totalAmount
      )
