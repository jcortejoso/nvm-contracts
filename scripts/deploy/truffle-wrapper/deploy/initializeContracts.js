...
async function initializeContracts({
  ...
  // testnet only!
  if (contracts.indexOf("Dispenser") > -1 && getAddress("Token")) {
    addressBook.Dispenser = zosCreate({
      contract: "Dispenser",
      network,
      args: [getAddress("Token"), roles.ownerWallet],
      verbose
    })
  }
  ...
  if (getAddress("ConditionStoreManager")) {
    if (contracts.indexOf("EscrowPaymentCondition") > -1) {
      addressBook.EscrowPaymentCondition = zosCreate({
        ...
        network,
        args: [roles.ownerWallet, getAddress("ConditionStoreManager")],
        verbose
      })
    }
  ...
  if (getAddress("ConditionStoreManager") && getAddress("DIDRegistry")) {
